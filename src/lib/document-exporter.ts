 ...
 *
 * IMPORTANT: `docx` uses Node.js APIs (Buffer, etc.) and MUST be loaded
 * via a dynamic import() — never a static top-level import. Static imports
 * bundle it into the browser JS and crash the page on load.
 *
 * Falls back to the server endpoint `/api/generate-document/docx` when
 * client-side Packer fails.
 *
...
/* ------------------------------------------------------------------ */
/*  Markdown → docx Paragraph[] builder  (runs INSIDE dynamic import) */
/* ------------------------------------------------------------------ */

/**
 * Build docx paragraphs from markdown text.
 * Receives the docx classes as parameters since this runs after the
...
function buildDocxParagraphs(
  ...
  function parseInlineRuns(line: string) {
  ...
  for (const rawLine of lines) {
    ...
    if (/^_{3,}/.test(line.trim())) {
      children.push(
        new Paragraph({
          children: [new TextRun({ text: '___________________________', font: 'Georgia' })],
...
/* ------------------------------------------------------------------ */
/*  Download as native .docx  (DYNAMIC import — safe for browser)      */
/* ------------------------------------------------------------------ */
...
export async function downloadAsWord(
  ...
  try {
    // DYNAMIC import — only loaded when user clicks Word export.
    // This keeps docx OUT of the initial browser bundle entirely.
    const docxModule = await import('docx')
    const { Document, Packer, AlignmentType } = docxModule

    const paragraphs = buildDocxParagraphs(text, docxModule)
